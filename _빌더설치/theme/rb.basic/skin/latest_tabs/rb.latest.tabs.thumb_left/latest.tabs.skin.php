   ...
   <!-- 탭 { -->
    ...
    <script>
        $(document).ready(function() {
            setTimeout(function () {
                var activeElement = document.querySelector('.swiper-container-tab-<?php echo $rb_skin['md_id']; ?> .active');
                var initialSlideIndex = 0;

                if (activeElement) {
                    var parentLi = activeElement.closest('li.swiper-slide-tab-<?php echo $rb_skin['md_id']; ?>');
                    var allSlides = document.querySelectorAll('li.swiper-slide-tab-<?php echo $rb_skin['md_id']; ?>');
    ...
    <?php foreach ($tabs as $i => $tab): ?>
      ...
      <div class="latest-tab-content<?php echo $i == 0 ? ' active' : ''; ?>" id="tab-<?php echo $rb_skin['md_id'].'-'.$i; ?>">
       ...
       <button type="button" class="more_btn" onclick="location.href='<?php echo $links_url ?>';" style="display:<?php echo (isset($rb_skin['md_title_hide']) && $rb_skin['md_title_hide'] == '1') ? 'none' : 'block'; ?>">더보기</button>
        ...
        <div class="rb_swiper"
          id="rb_swiper_<?php echo $rb_skin['md_id'].'_'.$i ?>"
              ...
              <?php foreach ($list as $row): ?>
                ...
                <div class="rb_swiper_list">
                  <div>
                    ...
                    <ul class="bbs_main_wrap_con_ul2" style="<?php echo !$rb_skin['md_thumb_is'] ? 'padding-left:0px !important; min-height:auto;' : '' ?>">
                      ...
                      <?php if($rb_skin['md_content_is']): ?>
                        <li class="bbs_main_wrap_con_cont">
                          ...
                          <?php else: ?>
                            <a href="<?php echo $wr_href ?>" class="cut2" style="min-height:36px;"><?php echo $wr_content ?></a>
                          <?php endif; ?>
                        </li>
                      <?php endif; ?>
                      ...
                      <?php if($rb_skin['md_icon_is']): ?>
                        <li class="bbs_main_wrap_con_writer">
                          <?php
                            if ($row['icon_new']) echo '<span class="bbs_list_label label3">새글</span>';
                            if ($row['icon_hot']) echo '<span class="bbs_list_label label1">인기</span>';
                          ?>
                        </li>
                      <?php endif; ?>
                    </ul>

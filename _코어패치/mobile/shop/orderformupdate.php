...
$cart_status = isset($cart_status) ? (string)$cart_status : '';
$tmp_cart_id = isset($tmp_cart_id) ? preg_replace('/[^0-9]/', '', (string)$tmp_cart_id) : '';

if ($cart_status === "입금") {
    // 1) ct_partner 컬럼 존재 여부 확인
    $has_ct_partner = false;
    $col_rs = sql_query("SHOW COLUMNS FROM {$g5['g5_shop_cart_table']} LIKE 'ct_partner'");
    if ($col_rs && sql_num_rows($col_rs) > 0) {
        $has_ct_partner = true;
    }

    // 2) 컬럼이 있을 때만 조회
    if ($has_ct_partner && $tmp_cart_id !== '') {
        $sqls = "SELECT DISTINCT ct_partner
                 FROM {$g5['g5_shop_cart_table']}
                 WHERE od_id = '".sql_escape_string($tmp_cart_id)."'";
        $results = sql_query($sqls, false); // 실패 시 종료 방지

        if ($results) {
            while ($rows = sql_fetch_array($results)) {
                if (isset($rows['ct_partner']) && $rows['ct_partner'] !== '') {
                    $partner_list[] = trim($rows['ct_partner']);
                }
            }
        }
    }
